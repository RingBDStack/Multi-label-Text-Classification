...
class Dataset(object):
    def __init__(self, filename, emb, classes, maxbatch = 32, maxword = 500 ):
        lines = map(lambda x: x.split('\t\t'), open(filename).readlines())
        # here i need more label. there is only one label            
        label = map(lambda x: x[0].split(' '), lines)
        oneslable = numpy.zeros([len(label), int(classes)], dtype=numpy.int32)
        for i in range(0,len(label)):
            for j in label[i]:
                oneslable[i][int(j)] = 1
        label = oneslable
        print("already done the ones-hot")
        docs = map(lambda x: x[1][0:len(x[1])-1], lines) 
